  ...
  Future<void> _submitTicket() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('http://10.0.2.2:3000/api/support/tickets'),
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        throw Exception('Error al enviar ticket: ${response.statusCode}');
      }
    } catch (e) {
      if (mounted) {
        _showErrorSnackBar('Error al enviar el mensaje. Intenta nuevamente.');
      }
    } finally {
      if (mounted) {
        setState(() => _isSubmitting = false);
